...
def rates(
    ...
    if len(physics)>0:
        ...
        # Calculate cross-sections at EBIT energies
        '''
        fac.CECross(
            name+'b.ce', name+'_EBIT.ce',
            -1, -1,
            [
...
def _def(key=None):
    ...
    if key.startswith('XEUS_Mo'):
        ...
        # Organized as "Survey_W_nele", assume nele = {11,18}
        nouter = int(float(key.split('Survey_W_')[-1])-10)
        if nouter < 3:
            outer = '3s%i'%(nouter)
        elif nouter < 9:
            outer = '3s2 3p%i'%(nouter-2)
        # Ionization state
        if nouter < 2:
            ioniz1 = '1*2 2*8'
        elif nouter < 4:
        ...
        ioniz2 = ''
        key = 'EBIT_W'
    elif key.startswith('v2_EBIT_W'):
        # Organized as "XEUS_Mo_nele"
        nouter = int(float(key.split('v2_EBIT_W_')[-1])-10)
        outer = ''
        ioniz1 = ''
        ioniz2 = ''
        key = 'v2_EBIT_W'
    elif key.startswith('W7X_W'):
        nouter = int(float(key.split('W7X_W_')[-1])-18)
        outer = ''
        ioniz1 = ''
        ioniz2 = ''
        key = 'W7X_W'
    ...
    dsettings = {
        ...
        'EBIT_W':{
            ...
            'exc':{             # --- Excited state --- # 
                ...
                'exc.3->4f': '1*2 2*8 3*%i 4f;3d<1'%(nouter-1+2),
                'exc.3->5f': '1*2 2*8 3*%i 5f;3d<1'%(nouter-1+2),
                'exc.3->6f': '1*2 2*8 3*%i 6f;3d<1'%(nouter-1+2),
                'exc.3->4d': '1*2 2*8 3*%i 4d1;3d<1'%(nouter+1),
                'exc.3->5d': '1*2 2*8 3*%i 5d1;3d<1'%(nouter+1),
                },
            'ion':{                 # --- Ionized state ---- #
        ...
        'v2_EBIT_W':{
            ...
            'grd':{                 # Number of electrons in each quantum number at ground
                ...
                #'grd.0': '1*2 2*8 3s2 3p%i'%(nouter-2),
                },
            'exc':{             # --- Excited state --- # 
                'exc.3p->4s': (
                    '1*2 2*8 3s2'
                ...
                #    + ' 9g1'
                #    ),
                #'exc.3s->9s': '1*2 2*8 3s1 3p%i 9s1'%(nouter-2),
                #'exc.3s->9p': '1*2 2*8 3s1 3p%i 9p1'%(nouter-2),
                #'exc.3s->9d': '1*2 2*8 3s1 3p%i 9d1'%(nouter-2),
                'exc.3p->10s': (
                    '1*2 2*8 3s2'
                    + (' 3p1' if nouter >3 else '')
                    + ' 10s1'
                ...
                #    + ' 10g1'
                #    ),
                #'exc.3s->10s': '1*2 2*8 3s1 3p%i 10s1'%(nouter-2),
                #'exc.3s->10p': '1*2 2*8 3s1 3p%i 10p1'%(nouter-2),
                #'exc.3s->10d': '1*2 2*8 3s1 3p%i 10d1'%(nouter-2),
                },
            'ion':{                 # --- Ionized state ---- #
                'ion.0': (
                    '1*2 2*8 3s2'
